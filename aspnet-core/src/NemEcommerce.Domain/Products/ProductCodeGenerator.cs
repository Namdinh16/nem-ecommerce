...
namespace NemEcommerce.Products
{
    public class ProductCodeGenerator : ITransientDependency
    {
        ...
        {
            string newCode;
            var identitySetting = await _identitySettingRepository.FindAsync(NemEcommerceConsts.ProductIdentitySettingId);
            if (identitySetting == null)
            {
                identitySetting = await _identitySettingRepository.InsertAsync(new IdentitySetting(NemEcommerceConsts.ProductIdentitySettingId, "Sản phẩm", NemEcommerceConsts.ProductIdentitySettingPrefix, 1, 1));
