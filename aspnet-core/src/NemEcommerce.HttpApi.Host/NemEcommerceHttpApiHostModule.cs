...
public class NemEcommerceHttpApiHostModule : AbpModule
{
        ...
        if (hostingEnvironment.IsDevelopment())
        {
            Configure<AbpVirtualFileSystemOptions>(options =>
            {
                ...
                options.FileSets.ReplaceEmbeddedByPhysical<NemEcommerceApplicationContractsModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}NemEcommerce.Application.Contracts"));
                options.FileSets.ReplaceEmbeddedByPhysical<NemEcommerceApplicationModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}NemEcommerce.Application"));
            });
        }
    }
    ...
    private void ConfigureConventionalControllers()
    {
        ...
        {
            options.ConventionalControllers.Create(typeof(NemEcommerceApplicationModule).Assembly);
        });
    }
    ...
    private static void ConfigureSwaggerServices(ServiceConfigurationContext context, IConfiguration configuration)
    {
        context.Services.AddAbpSwaggerGenWithOAuth(
            ...
            {
                    {"NemEcommerce", "NemEcommerce API"}
            },
            options =>
            {
                options.SwaggerDoc("v1", new OpenApiInfo { Title = "NemEcommerce API", Version = "v1" });
    ...
    public override void OnApplicationInitialization(ApplicationInitializationContext context)
    {
        ...
        {
            options.SwaggerEndpoint("/swagger/v1/swagger.json", "NemEcommerce API");
