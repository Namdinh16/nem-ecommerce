...
namespace NemEcommerce.Public.Catalog.Products
{
        ...
        PagedResultRequestDto>, IProductsAppService
    {
        ...
        public async Task<PagedResult<ProductAttributeValueDto>> GetListProductAttributeAsync(ProductAttributeListFilterDto input)
        {
            var attributeQuery = await _productAttributeRepository.GetQueryableAsync();
        ...
        public async Task<List<ProductInListDto>> GetListTopSellerProductAsync(int numberOfRecords)
        {
            var query = await Repository.GetQueryableAsync();
            query = query.Where(x => x.IsActive == true)
        ...
        {
            var product = await _productRepository.FirstOrDefaultAsync(x => x.Slug == slug);
