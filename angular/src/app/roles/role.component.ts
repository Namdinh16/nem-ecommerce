...
export class RoleComponent implements OnInit, OnDestroy {
  ...
  loadData(selectionId: string | null = null): void {
    ...
    let listData$: Observable<RoleInListDto[] | PagedResultDto<RoleInListDto>>;

    if (!this.keyword) {
        listData$ = this.roleService.getListAll();
    } else {
        const filterOptions = {
        ...
        listData$ = this.roleService.getListFilter(filterOptions);
    }

    listData$.pipe(takeUntil(this.ngUnsubscribe)).subscribe({
        next: (response: RoleInListDto[] | PagedResultDto<RoleInListDto>) => {
