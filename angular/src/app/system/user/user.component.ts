...
export class UserComponent implements OnInit, OnDestroy {
  ...
  loadData(selectionId: string | null = null): void {
    this.toggleBlockUI(true);

    let listData$: Observable<UserInListDto[] | PagedResultDto<UserInListDto>>;
    ...
    if (this.keyword) {
        ...
        listData$ = this.userService.getListWithFilter(filterOptions);
    } else {
        const pagingOptions = {
            skipCount: this.skipCount,
            maxResultCount: this.maxResultCount
        };
        listData$ = this.userService.getList(pagingOptions);
    }

    listData$.pipe(takeUntil(this.ngUnsubscribe)).subscribe({
        next: (response: UserInListDto[] | PagedResultDto<UserInListDto>) => {
            ...
            if (selectionId && this.items.length > 0) {
